...
int lru(int str_len, int str[], int no_f, int frame_arr[]){
        ...
        if( Present(str[i], frame_arr, no_f) == 1){     //checking if the page is present
            ...
            if( sp >= 0){
                ...
                //find the least recently used and remove
                int pick[100];
                int count =0;
                int ptr = i-1;
                tozero(pick, no_f);  //pick = [0, 0, 0, 0..]

                for(ptr = i-1; ptr >= 0; ptr--){
                    int find_val = str[ptr];
                    for(int j=0; j<no_f; j++){
                        if(frame_arr[j] == find_val){
                            if(pick[j] != 1){
                                pick[j] = 1;
                                count++;
                                if(count == no_f-1){
                                    //replace the index with 0 corresponding to frame_arr from pick
                                    int rep_ind = ret_zero(pick, no_f);
                                    frame_arr[rep_ind] = str[i];
                                }
                            }
                        }
                    }
...
int main() {
    ...
    lru(str_len, str, no_f, frame_arr);
